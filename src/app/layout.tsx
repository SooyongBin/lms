...
export default async function RootLayout({ children }: { children: ReactNode }) {
  ...
  if (!admin || (adminCount ?? 0) === 0) {
    ...
    if (session) {
      ...
      session = null;
      console.log('[01] 1. no admin data, session reset');
    }
    adminLinkText = '관리자 등록1';
    loginHref = '/login?mode=register';
  } else if (!session) {
    console.log('[01] 2. admin data, no session');
    ...
    await supabase.auth.signOut();
    session = null;
    console.log('[01] 3. admin data, uuid mismatch, session reset');
    // adminLinkText = '관리자';
    // loginHref = '/login';
  } else {
    console.log('[01] 4. admin data, uuid match');
